// src/games/CandySort/GameResultScreen.js
import { LinearGradient } from 'expo-linear-gradient';
import { StyleSheet, Text, TouchableOpacity, View } from 'react-native';
import { candyTheme, fontSizes, spacing } from '../../styles/theme';

export default function CandySortResultScreen({ navigation, route }) {
  ...
  const handlePlayAgain = () => {
    // Directly navigate back to Candy Sort game - NO gameId checking needed
    navigation.replace('CandySort', { 
  ...
  const handleLevelSelect = () => {
    navigation.navigate('CandySortLevelSelect', { 
      gameId: 'candy_sort' 
    });
  };
    ...
    <LinearGradient colors={[candyTheme.gradientStart, candyTheme.gradientEnd]} style={styles.container}>
      <View style={styles.content}>
        ...
        <View style={styles.gameInfo}>
          <Text style={styles.gameName}>🍬 Candy Sort</Text>
        ...
        {!isWin && (
          <View style={styles.encouragementContainer}>
            <Text style={styles.encouragementText}>💪 Don't give up! Try again and sort those candies correctly!</Text>
          </View>
        )}
