// src/games/CandyCount/GameResultScreen.js
import { LinearGradient } from 'expo-linear-gradient';
import { useEffect } from 'react';
import { StyleSheet, Text, TouchableOpacity, View } from 'react-native';
import { candyTheme, fontSizes, spacing } from '../../styles/theme';

export default function CandyCountResultScreen({ navigation, route }) {
  ...
  useEffect(() => {
    ...
    console.log('Received targetScore:', targetScore);
    console.log('Received stars:', stars);
    console.log('Received levelNumber:', levelNumber);
    console.log('Received isWin:', isWin);
    console.log('Percentage:', (score / targetScore) * 100, '%');
  }, []);
  ...
  const handlePlayAgain = () => {
    navigation.replace('CandyCount', { 
      levelNumber: levelNumber, 
      gameId: 'candy_count',
      timeLimit: Math.max(30, 60 - Math.floor(levelNumber / 2)),
      targetScore: 50 + (levelNumber * 10),
    });
  };
  ...
  const handleNextLevel = () => {
    navigation.replace('CandyCount', { 
      ...
      gameId: 'candy_count',
      timeLimit: Math.max(30, 60 - Math.floor(nextLevel / 2)),
      targetScore: 50 + (nextLevel * 10),
    });
  };

  const handleLevelSelect = () => {
    navigation.navigate('CandyCountLevelSelect', { 
      gameId: 'candy_count' 
    });
  };
      ...
      <View style={styles.content}>
        ...
        <View style={styles.scoreContainer}>
          ...
          <Text style={styles.percentage}>
            {((score / targetScore) * 100).toFixed(1)}% of target
          </Text>
        </View>
        ...
        <View style={styles.gameInfo}>
          <Text style={styles.gameName}>🔢 Candy Count</Text>
          <Text style={styles.levelText}>Level {levelNumber}</Text>
        </View>

        <View style={styles.buttonContainer}>
          {isWin && isNextLevelUnlocked && nextLevel <= 30 && (
