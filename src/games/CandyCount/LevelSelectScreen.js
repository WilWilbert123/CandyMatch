// src/games/CandyCount/LevelSelectScreen.js
...
// Generate Candy Count levels (30 levels)
const generateCandyCountLevels = () => {
  const levels = [];
  for (let i = 1; i <= 30; i++) {
    levels.push({
      id: i,
      ...
      targetScore: 50 + (i * 10),
      maxNumber: Math.min(20, 5 + Math.floor(i / 5)),
    });
  }
  return levels;
};

const CANDY_COUNT_LEVELS = generateCandyCountLevels();

export default function LevelSelectScreen({ navigation, route }) {
  const { gameId = 'candy_count' } = route.params || {};
  const [unlockedLevels, setUnlockedLevels] = useState([1]);
  const [selectedWorld, setSelectedWorld] = useState(1);
  const [levelStars, setLevelStars] = useState({});
  const [loading, setLoading] = useState(true);

  // Create worlds (10 levels per world for 30 total levels)
  const worlds = [];
  for (let i = 0; i < CANDY_COUNT_LEVELS.length; i += 10) {
    worlds.push({
      id: i / 10 + 1,
      levels: CANDY_COUNT_LEVELS.slice(i, i + 10),
      startLevel: i + 1,
      endLevel: Math.min(i + 10, CANDY_COUNT_LEVELS.length)
    });
  }
  ...
  const loadData = async () => {
    ...
    try {
      ...
      console.log('Full progress from storage:', progress);
      
      // Get unlocked levels from storage.js
      const unlocked = progress.unlockedLevels || [1];
      setUnlockedLevels(unlocked);
      
      // Get stars from storage.js
      const stars = progress.bestStars || {};
      setLevelStars(stars);

      console.log('Loaded Candy Count levels:', { unlocked, stars });
    } catch (error) {
      console.error('Error loading Candy Count data:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const renderLevel = ({ item: level }) => {
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() => {
          if (isUnlocked) {
            navigation.navigate('CandyCount', {
