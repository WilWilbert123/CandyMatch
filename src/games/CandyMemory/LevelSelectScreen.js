// src/games/CandyMemory/LevelSelectScreen.js
...
// Generate Candy Memory levels (50 levels)
const generateCandyMemoryLevels = () => {
  const levels = [];
  for (let i = 1; i <= 50; i++) {
    levels.push({
      id: i,
...
const CANDY_MEMORY_LEVELS = generateCandyMemoryLevels();

export default function LevelSelectScreen({ navigation, route }) {
  const { gameId = 'candy_memory' } = route.params || {};
  ...
  const worlds = [];
  for (let i = 0; i < CANDY_MEMORY_LEVELS.length; i += 10) {
    worlds.push({
      id: i / 10 + 1,
      levels: CANDY_MEMORY_LEVELS.slice(i, i + 10),
      startLevel: i + 1,
      endLevel: Math.min(i + 10, CANDY_MEMORY_LEVELS.length)
    });
  }
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const stars = {};
      for (let i = 1; i <= CANDY_MEMORY_LEVELS.length; i++) {
        stars[i] = progress.bestStars?.[i] || 0;
      }
      setLevelStars(stars);
      
      console.log('Loaded Candy Memory levels:', { unlocked, stars });
    } catch (error) {
      console.error('Error loading Candy Memory data:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const renderLevel = ({ item: level }) => {
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() => {
          if (isUnlocked) {
            navigation.navigate('CandyMemory', { 
